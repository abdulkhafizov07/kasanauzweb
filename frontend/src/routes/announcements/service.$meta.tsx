...
import { ServiceAnnouncementCardWidget } from '@/components/web/announcements/service-card'
...
export const Route = createFileRoute('/announcements/service/$meta')({
...
function RouteComponent() {
  ...
  const { meta } = useParams({ from: '/announcements/service/$meta' })
  ...
  const {
    ...
    queryKey: ['service-announcement', 'details', meta],
    queryFn: async () => {
  ...
  const handleSaveAnnouncement = async () => {
    ...
    try {
      await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/annnouncements/api/save/`,
        {
          guid: currentAnnounce.guid,
        },
      )
      refetch() // refresh `saved` state
    } catch (err) {
      ...
      <div className="grid grid-cols-3 gap-4 py-6">
        ...
        <div className="left">
          <div className="flex flex-wrap justify-between gap-y-3">
            {announcements.map((val) => (
              <ServiceAnnouncementCardWidget key={val.guid} value={val} />
            ))}
          </div>
        </div>
        ...
        <div className="right col-span-2">
          <div className="bg-white rounded-lg p-4 pt-0 sticky top-18 min-h-[calc(100vh-221px)] overflow-y-auto">
            <div className="w-full aspect-[13/6] overflow-hidden rounded-lg mb-4">
              <img
                src={currentAnnounce.thumbnail}
                alt=""
              ...
              <div className="flex gap-2">
                {isAuthenticated ? (
                  <>
                    ...
                    <Button onClick={handleConnect}>Bog‘lanish</Button>
                  </>
                ) : (
                  <Button asChild>
            ...
            <ul className="flex flex-col md:flex-row md:justify-between gap-4 my-5">
              ...
              <li className="flex items-center gap-3">
                <ClockIcon size={24} />
                <div>
                  <p className="text-text">Xizmat vaqti</p>
                  <p className="font-semibold text-text">
