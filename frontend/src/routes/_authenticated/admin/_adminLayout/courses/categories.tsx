  ...
  component: () => {
    ...
    useEffect(() => {
      ...
      if (!(auth.isLoading || auth.isUserLoading)) {
        ...
        if (
          !(permissions || hasOne(permissions, 'courses:read_categories')) &&
...
const MoreButton: React.FC<{
  info: CellContext<CourseCategory, unknown>
  refetch: () => Promise<
    QueryObserverResult<
      {
...
}> = ({ info, refetch }) => {
  {
    ...
    const data = info?.row.original as CourseCategory
        ...
        <DropdownMenuContent>
          <DropdownMenuGroup>
            ...
            <DropdownMenuItem
              disabled={
                data.state === 'approved' ||
                !hasOne(user?.permissions || {}, 'courses:write_categories')
              }
            >
              {commonT('approved')}
            </DropdownMenuItem>
          ...
          <DropdownMenuGroup>
            <DropdownMenuItem
              disabled={
                !hasOne(user?.permissions || {}, 'courses:delete_category')
              }
              onClick={(e) => {
                e.preventDefault()
                e.stopPropagation()
                ...
                api
                  .delete(
                    ...
                    }/courses/api/dashboard/categories/${data.guid}/`,
                  )
                  .finally(() => {
                    if (refetch) {
                      refetch()
            ...
            <DropdownMenuItem
              disabled={
                !hasOne(user?.permissions || {}, 'courses:write_category')
              }
            >
              {commonT('edit')}
            </DropdownMenuItem>
...
function RouteComponent() {
  const { t } = useTranslation(undefined, {
    keyPrefix: 'admin.courses.categories',
  })
  const { t: createT } = useTranslation(undefined, {
    keyPrefix: 'admin.courses.categories.create',
  })
  const { t: commonT } = useTranslation(undefined, {
    keyPrefix: 'admin.table.common',
  })
  ...
  const fetchData = async (): Promise<PaginatedResponse<CourseCategory>> => {
    const params = buildQueryParams(
      debouncedSorting,
      debouncedColumnFilters,
      debouncedPagination,
    )
    const response = await api.get<PaginatedResponse<CourseCategory>>(
      `/courses/api/dashboard/categories/?${params}`,
    )
    ...
    onSubmit: async ({ value }) => {
      try {
        const res = await api.post(
          `${
            import.meta.env.VITE_BACKEND_URL
          }/courses/api/dashboard/categories/`,
          value,
        )
          ...
          <SheetContent>
            ...
            <div className="flex flex-col space-y-4 px-4">
              <RenderField
                ...
                labelKey="title"
                translationOptions={{
                  keyPrefix: 'admin.courses.categories.create.fields',
                }}
